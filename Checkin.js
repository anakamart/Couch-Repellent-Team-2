      ...
      if (response.ok) {
        ...
        if (responseText.trim()) {
          try {
            ...
            Alert.alert('Error', 'Failed to parse existing data.');
            return;
          }
        }

        // Handles merging data if existing data, or new data if empty
        const updatedData = existingData && Object.keys(existingData).length > 0
